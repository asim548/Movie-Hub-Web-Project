...
import { FaTachometerAlt, FaFilm, FaSearch, FaUserCog, FaBell } from 'react-icons/fa';
...
function SellerHeader() {
    ...
    const [profile, setProfile] = useState(() => {
        ...
        return { name: cached?.name || 'Seller', photo: cached?.photo || '' };
    });

    useEffect(() => {
        getCurrentProfile()
            .then((data) => setProfile({ name: data?.name || 'Seller', photo: data?.photo || '' }))
            .catch(() => {});
            ...
            <div className="mh-container py-3 flex items-center gap-3">
                <div className="shrink-0">
                    <div>
                        ...
                        <p className="font-bold">Seller Portal</p>
                    </div>
                </div>
                <nav className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 overflow-x-auto whitespace-nowrap [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                        ...
                        />
                        <Button
                            onClick={() => navigator('/seller/persons')}
                            icon={<FaUserCog />}
                            label="Cast & Crew"
                        />
                        <Button
                            onClick={() => navigator('/seller/notifications')}
                            icon={<FaBell />}
                            label="Notifications"
                        />
                    </div>
                ...
                <div className="shrink-0 flex items-center gap-2">
                    <button
                        onClick={() => navigator('/seller/management')}
                    ...
                    >
                        ...
                        <span className="truncate">{profile.name || 'Seller'}</span>
                    </button>
                    <button
