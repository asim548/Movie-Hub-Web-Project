...
const createReview = async (req, res) => {
    try {
        ...
        const reviewUser = await User.findById(user).select('email name');
        if (reviewUser?.email) {
            await sendMail(
                reviewUser.email,
                `Review added: ${foundMovie.title || 'Movie'}`,
                `Hi ${reviewUser.name || 'there'}, your review (${rating}/5) was posted successfully for "${foundMovie.title || 'the movie'}".`
            );
        }
...
const updateReview = async (req, res) => {
    try {
        ...
        if (reviewUser?.email) {
            await sendMail(
                reviewUser.email,
                `Review updated: ${foundMovie.title || 'Movie'}`,
                `Hi ${reviewUser.name || 'there'}, your review was updated for "${foundMovie.title || 'the movie'}". New rating: ${rating}/5.`
            );
        }
...
const deleteReviewByAdmin = async (req, res) => {
    ...
    try {
        ...
        const foundMovie = await Movie.findById(review.movie).select('title');
        const reviewUser = await User.findById(review.user).select('email name');
        ...
        if (reviewUser?.email) {
            await sendMail(
                reviewUser.email,
                `Review removed: ${foundMovie?.title || 'Movie'}`,
                `Hi ${reviewUser.name || 'there'}, your review for "${foundMovie?.title || 'the movie'}" was removed by admin.`
            );
        }
