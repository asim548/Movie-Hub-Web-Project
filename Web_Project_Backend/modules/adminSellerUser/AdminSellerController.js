...
const registerAdminSeller = async (req, res) => {
    ...
    totalPaid:0,
        ...
        await sendMail(
            newUser.email,
            "Welcome to MovieHub",
            `Hi ${newUser.name || "there"}, your ${role} account has been created successfully on MovieHub.`
        );
...
const getProfileAdminSeller = async (req, res) => {
    try {
        ...
        // Find the user by ID
        const user = await AdminSeller.findById(id, "-password -refreshToken");
        if (!user) {
...
const updateProfileAdminSeller = async (req, res) => {
    try {
        const { _id, name, email, password, photo, themeMode } = req.body;
        ...
        if (typeof photo === 'string') updateData.photo = photo;
        if (themeMode === 'dark' || themeMode === 'light') updateData.themeMode = themeMode;
        if (password) {
            updateData.password = await bcrypt.hash(password, 10);
        }

        // Update user profile
        const updatedUser = await AdminSeller.findByIdAndUpdate(userId, updateData, {
            new: true,
            runValidators: true
        });
...
const sellerDashboard = async(req, res) => {
    ...
    const seller = await AdminSeller.findById(req.params.id).select("-password -refreshToken");
    console.log(seller)
